...
func TestToolDetailDialogFallsBackToUnifiedDiffForAsymmetricEditBlock(t *testing.T) {
	defaultTheme := theme.StaticDefault()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
			...
			"turn-1": {
				...
				ToolCalls: map[string]*events.ToolCallState{
					"call-1": {
						CallID:    "call-1",
						ToolName:  "edit",
						Input:     `{"path":"src/database.ts","old_text":"await mongoose.connect(uri);\n","new_text":"await mongoose.connect(uri, {\n  maxPoolSize: parseInt(process.env.MONGO_MAX_POOL_SIZE || '50'),\n  minPoolSize: parseInt(process.env.MONGO_MIN_POOL_SIZE || '10'),\n  maxIdleTimeMS: 30000,\n  serverSelectionTimeoutMS: 5000,\n  socketTimeoutMS: 45000,\n});\n"}`,
						Output:    "edited line 8 in /repo/src/database.ts",
						Declared:  true,
						Completed: true,
						Succeeded: true,
						WriteMutation: &events.WriteMutation{
							Path:    "/repo/src/database.ts",
							Existed: true,
							Before:  "const uri = process.env.MONGO_URI || 'mongodb://localhost:27017/projectdb';\nawait mongoose.connect(uri);\n};\n",
							DiffPreview: &textdiff.Preview{
								OldStartLine: 8,
								NewStartLine: 8,
								Ops: []textdiff.PreviewOp{
									{Kind: textdiff.OpContext, Text: "const uri = process.env.MONGO_URI || 'mongodb://localhost:27017/projectdb';"},
									{Kind: textdiff.OpDelete, Text: "await mongoose.connect(uri);"},
									{Kind: textdiff.OpInsert, Text: "await mongoose.connect(uri, {"},
									{Kind: textdiff.OpInsert, Text: "  maxPoolSize: parseInt(process.env.MONGO_MAX_POOL_SIZE || '50'),"},
									{Kind: textdiff.OpInsert, Text: "  minPoolSize: parseInt(process.env.MONGO_MIN_POOL_SIZE || '10'),"},
									{Kind: textdiff.OpInsert, Text: "  maxIdleTimeMS: 30000,"},
									{Kind: textdiff.OpInsert, Text: "  serverSelectionTimeoutMS: 5000,"},
									{Kind: textdiff.OpInsert, Text: "  socketTimeoutMS: 45000,"},
									{Kind: textdiff.OpInsert, Text: "});"},
									{Kind: textdiff.OpContext, Text: "};"},
								},
							},
						},
					},
	...
	rendered := renderTestDialogContentPlain(dialog)
	if strings.Contains(rendered, "Old") || strings.Contains(rendered, "New") {
		t.Fatalf("asymmetric edit block should fall back to unified diff\nrendered:\n%s", rendered)
	}
	for _, want := range []string{
...
func TestToolDetailDialogTabForcesSplitViewForAsymmetricEditBlock(t *testing.T) {
	defaultTheme := theme.StaticDefault()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	dialog := newToolDetailDialog(model, state, sessionToolCallRef{TurnID: "turn-1", CallID: "call-1"}, state.Turns["turn-1"].ToolCalls["call-1"])
	initial := renderTestDialogContentPlain(dialog)
	if strings.Contains(initial, "Old") || strings.Contains(initial, "New") {
		t.Fatalf("initial dialog should use unified diff\nrendered:\n%s", initial)
	}

	updated, _ := dialog.Update(tea.KeyPressMsg{Code: tea.KeyTab})
	next := updated.(*toolDetailDialog)
	rendered := renderTestDialogContentPlain(next)
	...
	for _, want := range []string{
		...
		if !strings.Contains(rendered, want) {
			t.Fatalf("split dialog missing %q\nrendered:\n%s", want, rendered)
		}
	}
}

func TestToolDetailDialogOmitsRedundantEditHeaderBeforeDiff(t *testing.T) {
	defaultTheme := theme.StaticDefault()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	dialog := newToolDetailDialog(model, state, sessionToolCallRef{TurnID: "turn-1", CallID: "call-1"}, state.Turns["turn-1"].ToolCalls["call-1"])
	rendered := renderTestDialogContentPlain(dialog)
	if strings.Count(rendered, "src/app.ts") != 1 {
		t.Fatalf("dialog should mention edit path once via title only\nrendered:\n%s", rendered)
	}
	for _, unwanted := range []string{
		...
		if strings.Contains(rendered, unwanted) {
			t.Fatalf("dialog unexpectedly repeated edit heading detail %q\nrendered:\n%s", unwanted, rendered)
		}
	}
	if !strings.Contains(rendered, "exact text match") {
		t.Fatalf("dialog missing condensed edit context line\nrendered:\n%s", rendered)
	}
	for _, want := range []string{
		"Old",
		"New",
...
func TestToolDetailDialogShowsFailedEditDetailsOnlyInPopup(t *testing.T) {
	defaultTheme := theme.StaticDefault()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
			...
			"turn-1": {
				...
				ToolCalls: map[string]*events.ToolCallState{
					"call-1": {
						CallID:    "call-1",
						ToolName:  "edit",
						Input:     `{"path":"src/models/Comment.ts","start_line":100,"old_text":"return depth;\n","new_text":"return Math.min(depth, 5);\n"}`,
						Error:     "Read src/models/Comment.ts line 100, then retry edit.",
						Declared:  true,
						Completed: true,
						Succeeded: false,
					},
	...
	for _, want := range []string{
		"src/models/Comment.ts",
		"line 100",
		"exact text match from line 100",
		"Read src/models/Comment.ts line 100, then retry edit.",
	} {
		if !strings.Contains(rendered, want) {
			t.Fatalf("dialog missing %q\nrendered:\n%s", want, rendered)
		}
	...
	for _, unwanted := range []string{
		...
		if strings.Contains(rendered, unwanted) {
			t.Fatalf("dialog includes noisy failed edit detail %q\nrendered:\n%s", unwanted, rendered)
		}
	}
	if strings.Contains(rendered, "Structured details unavailable.") {
		t.Fatalf("dialog unexpectedly fell back to generic details\nrendered:\n%s", rendered)
