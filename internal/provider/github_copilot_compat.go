...
// support small-model coercions (for example integer-or-string fields), richer
// branching contracts, and object-map inputs. Do not weaken those tool definitions just to satisfy one
// transport. Instead, keep the runtime contract authoritative and adapt only
// the Copilot wire payload down to the provider-safe subset here.
