...
![Shell layout showing a single transcript plane, compact tool rows, and no persistent inspector sidebar.](/screenshots/tui-shell-layout.svg)

Shell view is a single-plane terminal workflow. It keeps the transcript,
composer, status, workflow activity, and compact tool rows in one continuous
surface.
