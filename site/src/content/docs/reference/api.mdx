...
| `GET` | `/sessions/:id/traces` | Get persisted per-turn step traces (requires `session.trace: true`) |
...
| `step_trace` | Per-turn step breakdown for `/cost` and `/trace` (only when `session.trace: true`) |
...
When trace capture is enabled, `step_trace` events and `GET /sessions/:id/traces` let you build external dashboards or postmortem tooling around slow or expensive workflows.
