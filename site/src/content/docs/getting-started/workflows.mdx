...
`/workflow <id>` selects a workflow for later turns, `/workflow off` clears the
selection, and `/workflow resume` resumes a blocked workflow phase. If a
workflow is active, the footer shows the workflow id, current phase, and blocked
reason when one exists. `/trace` shows phase history and recorded results.
...
`max_revision_loops` caps automatic revision loops after failed verification or
review evidence. A transition-specific `max_loops` overrides that workflow
default for the matching transition. Approval phases can use
