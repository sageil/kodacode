...
- Debug workflow: use `debug` for reproduce, identify, patch, regression, verify, review
...
| `type: review` | Structured review evidence and optional parallel review passes | `agent`, `mode`, `review_passes`, `parallel_review`, `requires` |
...
JSON object with those keys. Missing or invalid fields block moving to the next
phase instead of being inferred from plain text. Verification phases with declared
`commands` run those commands through the runtime-owned `test` tool, so normal
execution permissions and check results apply.
...
types such as `git_diff`, `verification_result`, and `review_outcome`.
...
summaries combine all recorded review outcomes instead of using only the latest
review result. When `parallel_review: true` is set, the runtime runs one limited
review session for each declared pass and records each result as workflow data.
...
phases: # Ordered workflow phases.
  ...
  - id: implement # Stable phase id for the edit step.
    ...
    completion: # Evidence needed before leaving this phase.
      requires: # Completion evidence requirements for implementation.
        - git_diff # Require recorded diff evidence before verification and review.
    tools: # Narrow normal tools for this phase.
      allow: # Allow only this tool subset, subject to agent and sandbox policy.
        ...
        - git_diff # Permit diff inspection.
  ...
  - id: review # Stable phase id for review.
    ...
    parallel_review: true # Run one limited review session per review pass.
    review_passes: # Named review lenses included in the review prompt and final result.
      - id: correctness # Review pass id for implementation correctness.
        description: Behavior changes and whether the implementation is correct. # Prompt text for this pass.
      - id: tests # Review pass id for test coverage.
        description: Test coverage, edge cases, and missing checks. # Prompt text for this pass.
      - id: architecture # Review pass id for design and boundaries.
        description: APIs, config, permissions, compatibility, and system boundaries. # Prompt text for this pass.
    requires: # Evidence needed before entering review.
      - git_diff # Require recorded diff evidence.
      - verification_result # Require recorded verification evidence.
  ...
  - id: patch # Stable phase id for the fix.
    ...
    tools: # Tool surface for patching.
      allow: # Allow the tools needed for inspection, edits, and local checks.
        ...
        - git_diff # Permit diff inspection.
        - task_workflow # Permit workflow-bound task tracking.
  - id: verify # Stable phase id for regression checks.
    type: verification # Runtime runs declared commands and records check results.
    agent: engineer # Agent identity used for phase configuration and policy.
    commands: # Declared checks for this workflow.
      - tool: test # Use the test tool.
        command: go test ./... # Run the repository Go test suite.
    required: true # Require passing verification before advancing.
  - id: review # Stable phase id for read-only review.
    ...
    review_passes: # Review lenses for this bug fix.
      - id: cause # Review pass id for cause coverage.
        description: Whether the patch fixes the reproduced cause. # Prompt text for this pass.
      - id: regression # Review pass id for regression coverage.
        description: Test coverage and edge cases for the failure. # Prompt text for this pass.
    requires: # Evidence needed before review.
      - verification_result # Require recorded verification data.
...
phases: # Ordered review phases.
  ...
  - id: review # Stable phase id for structured review.
    ...
    parallel_review: true # Run each review pass in a limited review session.
