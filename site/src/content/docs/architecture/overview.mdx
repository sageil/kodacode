...
1. **SandboxMiddleware**: sets up tool execution context with the working directory and environment
...
3. **CompactionMiddleware**: checks context usage and triggers pruning or compaction when needed
...
6. **LLMMiddleware**: runs the streaming chat loop with parallel tool execution and fallback handling
...
- **Sessions**: metadata, model, agent, and title
- **Messages**: role, content, and tool calls/results
- **Message parts**: typed content such as text, tool calls, files, and images
...
- **App**: top-level model routing between the home and session views
- **Messages**: scrollable viewport with per-message render caching
- **Tool panels**: expandable blocks with diff rendering and syntax highlighting
- **Footer**: input textarea with slash command autocomplete
- **Header**: session info, agent, model, and variant display

Render performance uses dirty-flag tracking, so only changed messages are re-rendered and tool output is batched.
