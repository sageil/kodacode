...
KodaCode exposes agents as explicit runtime definitions, not prompt-only
personas. Agent markdown can narrow tool access, select a primary model, and add
prompt text, while runtime code still owns workflow phases, permissions, tool
...
The structured workflow path is centered on `engineer`, `planner`, and
`reviewer`.
...
- workflow review phases use `workflow_review_result` to record required
...
- `workflow.review_mode: manual`: review stays agent-driven; runtime does not
  add a follow-up review turn automatically
...
- `model`:
  ...
  session model.
- `mode`:
  decides whether the agent is selectable in the picker, reserved as a
  subagent-only definition, or both.
- `hidden`:
...
- `all`: selectable in the normal agent picker and also marked as compatible
  with runtime paths that require subagent eligibility
...
Agent markdown can include a `handoff:` frontmatter block. Current runtime
