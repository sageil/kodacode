...
The engineer, builder, and adviser agents are `primary` — shown in the picker and Tab cycling. The explorer, planner, polish, refactor, reviewer, and insight agents are `subagent` mode — they're invoked via `/<agent-name>` commands or by the model's subagent tool.
