...
Predefined providers (like Groq, DeepSeek, etc.) don't need a `base_url` in config — it's filled automatically. You only need `base_url` if overriding the default.
