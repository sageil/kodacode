...
- `session.tool_call_argument_timeout` defaults to `60s`, which is a good general default for hosted providers. If you use slower local models in Ollama or LM Studio and they take longer to finish emitting tool-call JSON, raise it globally or set a per-model override.
