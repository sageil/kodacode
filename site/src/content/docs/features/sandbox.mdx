...
When a tool requests access outside its default authority, KodaCode shows a saved approval prompt. The tool is paused until a decision is made.
...
| `ask` | Use the normal saved approval flow |
...
In `full_access`, agent tool access (`AllowTools`/`DisallowedTools`) still applies, but the runtime does not stop on policy rules or ordinary approval prompts.
...
Agent tool access (`AllowTools`/`DisallowedTools` in agent frontmatter) is a separate layer that controls which tools an agent is offered at all. It runs before this flow and is covered in the [Agents](/features/agents/) documentation.
