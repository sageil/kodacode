...
Every tool call in KodaCode is sandboxed to your project directory by default. File reads, writes, searches, and shell commands are confined to that boundary. Access outside the project requires explicit user permission.
...
Content commands such as `echo` and `printf` have their arguments skipped entirely. Only redirect targets are checked.
...
MCP tools, prefixed with their server name, are dispatched directly to their MCP server **without** path confinement. The MCP server is responsible for its own access control.

<Aside type="tip">
The sandbox is not process isolation, and it does not use containers. It is path-level access control with permission prompts, designed for developer workflows where tools run in the user's environment but should not silently touch files outside the project.
</Aside>
...
- **Grep/sed patterns**: quoted patterns containing `/` characters (for example `grep "// @ts-ignore"`) are not flagged unless the path exists on disk. If a grep pattern happens to match an existing external path (for example `grep '/usr/local/bin'`), you may see a prompt. Approve it and continue.
