...
KodaCode supports session and cross-session limits under `sessions` in
`config.yaml`:
...
- `budgets.warn_threshold: 0.8` means "warn when the active workflow reaches 80% of its workflow budget"
...
When a session, workflow, or cross-session budget is exceeded, KodaCode blocks
the next provider-backed turn before a provider call is made.
...
Budget settings are validated when config and workflow definitions are loaded:
...
- workflow `budgets.max_cost` must be non-negative
- workflow `budgets.warn_threshold` must be between `0` and `1` and requires `budgets.max_cost > 0`
- workflow `budgets.max_provider_requests_per_turn` must be non-negative
