...
## Turn Inspection With Trace

Enable trace capture when you want to know why a turn was expensive or slow, not just how much the session cost in total:
...
- `/cost` adds a per-turn table so you can spot the outlier turn quickly
- `/trace N` drills into one turn's step-by-step token usage, cache hits/writes, retries, fallbacks, prompt segment mix, and tool timing
- `GET /sessions/:id/traces` exposes the same persisted data over the API

**Practical value**:

- Find out whether a spike came from large file reads, repeated tool loops, retries, or prompt growth
- Separate model latency from tool latency before you start tuning the wrong thing
- Verify that compaction, pruning, or utility-model routing is actually reducing spend in your real workflow
- Compare recurring workflows and tune them with evidence instead of guesswork

## Budget Caps

Set a maximum dollar cost per session and, if needed, across all persisted sessions in your local database:
...
| Field | Effective Default | Description | Practical Value |
|-------|-------------------|-------------|-----------------|
| `budget` | `0` | Maximum dollars per session. `0` means unlimited. | Use this for per-task guardrails, CI jobs, provider experiments, or any session where you want a hard stop before spend runs away. |
| `budget_warn` | `0` | Fraction of `budget` at which a warning is shown. `0` disables warnings. | Set this to `0.8` or `0.9` to get an early warning while you still have time to change models, trim context, or stop. |
| `total_budget` | `0` | Maximum dollars across all persisted non-ephemeral sessions. `0` means unlimited. | Use this when several sessions or subagents share one budget and you need a global ceiling, not just per-session caps. |
| `total_budget_warn` | `0` | Fraction of `total_budget` at which a warning is shown. `0` disables warnings. | Helps catch aggregate spend pressure before multiple sessions collectively hit the global cap. |

When the per-session budget is exceeded, the tool loop stops. Tools are removed and the model must respond with what it has. When the cross-session budget is exceeded, the same stop behavior is triggered based on total persisted spend across sessions.
...
- **Explorer/insight subagents**: read-only research tasks that need comprehension, not generation

These are routed to the `utility_model`, a fast and inexpensive model that you configure once:
...
**Impact**: title generation and compaction summaries typically use 500–2,000 tokens each. Routing them to Haiku instead of Opus or Sonnet can reduce their cost by 10–30x per call.
...
1. **Per-model config**: only models with `thinking_budget` set use extended thinking at all. Models without it skip thinking entirely, which avoids unnecessary latency and cost.
2. **Auto-reduce on tool turns**: after the model's initial response, reasoning drops to 3K tokens for later tool-routing turns because the model is deciding which tool to call next, not solving the full problem.
...
Session costs are persisted to SQLite. When you resume a session, the accumulated cost is restored. The budget applies per session, so starting a new session resets the counter.
...
<Aside type="tip">
Use `/cost` during a session to see what's driving spend. If a specific turn looks wrong, enable `session.trace: true` and inspect it with `/trace N`.
</Aside>
