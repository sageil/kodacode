...
KodaCode supports the [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) for extending its capabilities with external tool servers. MCP tools appear alongside built-in tools, and the model uses them like any other tool.
...
Restart KodaCode after changing MCP configuration. Servers start concurrently on launch. Failed servers are logged, but they do not block startup.
...
**Server fails to start**: check that the command is installed and available in your `PATH`. Run it manually to verify: `npx -y @modelcontextprotocol/server-filesystem /tmp`

**Tools not appearing**: verify that the server name in `config.yaml` matches what you expect. MCP tools are prefixed, so a server named `filesystem` with a tool `read` appears as `mcp_filesystem_read`.

**Permission denied**: MCP tools default to `ask`. Add explicit `allow` rules in your permission config for tools you trust.

**Environment variables not expanding**: make sure the variable is exported in your shell (`export GITHUB_TOKEN=...`). KodaCode reads the process environment at startup.

## Best Practices

### Server Selection
- Use **stdio** transport for local tools. It is simpler to set up and avoids network overhead.
...
- MCP servers initialize in the background, so kodacode is usable immediately while they connect
- If a server fails to connect, kodacode continues without it. Check the log for details.

### Performance
- MCP startup is non-blocking, so kodacode is usable immediately while servers connect
