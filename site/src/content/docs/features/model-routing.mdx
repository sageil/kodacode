...
Phase `model` wins over workflow `model`. When neither is declared, KodaCode
uses the selected agent, session, and global config routing. For reviewer
phases, a declared workflow or phase model wins over `workflow.review_model`;
otherwise `workflow.review_model` remains the reviewer fallback.
