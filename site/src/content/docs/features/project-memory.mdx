...
KodaCode has real saved memory. It is not one feature hiding behind one file.

There are three different layers:

| Layer | Scope | Stored at | Best for |
| --- | --- | --- | --- |
| Global instructions | All repos on one machine | `~/.config/kodacode/AGENTS.md` | Personal defaults and saved cross-project rules |
| Project instructions | One repository | `<workspace>/AGENTS.md` | Repo policy, architecture constraints, and contributor guidance |
| Project memory | One repository | `<workspace>/.kodacode/memories/*.md` | Saved facts, decisions, and non-obvious context worth carrying across sessions |
...
- saved safety or review expectations
...
Do not use it as a dump for temporary task notes. If the information will be stale in a day or two, it probably belongs in the current session, not in saved project memory.

`/compress` also rewrites existing project memory entries when it can make them shorter without dropping substantive facts. This is a saved optimization step, not transient prompt compaction.
...
That means project memory is saved, but still bounded so it does not quietly take over the prompt.
...
In other words, project memory is for saved facts and decisions, not for storing full deliverables.
...
2. save a short project memory entry with the saved takeaway and the file path

Good project memory example:

```md
Performance optimization plan stored in docs/performance-plan.md.
Saved takeaways: API hot paths are project list aggregation and dashboard queries; avoid reintroducing N+1 reads; prefer cached summary endpoints for dashboard cards.
```
...
- saved compression: `/compress` rewrites the workspace-owned source files themselves

That distinction matters. Prompt compaction lowers tokens for one request shape. `/compress` lowers the steady-state size of the actual saved inputs that future turns load from disk.

## What gets loaded into a turn

For saved user and repository guidance, the current prompt assembly order is:
...
See [Skills](/features/skills/) for file layout, mention syntax, and examples.
